...
//! no entry, is not an installed program, and inventing one would put rudbman
...
//! `%LOCALAPPDATA%\Programs\rudbman` and a zip unpacked wherever the user
...
//! and the only thing written outside the directory rudbman is already
...
mod tests {
    ...
    impl ScratchKey {
        fn new() -> Self {
            let path = format!("Software\\rudbman-test-{}", uuid::Uuid::new_v4());
    ...
    fn an_entry_that_is_not_there_is_not_created() {
        ...
        let installed = tempfile::tempdir().expect("a temporary directory");
        let absent = format!("Software\\rudbman-test-{}", uuid::Uuid::new_v4());
