...
use i18n::ts;
use pane_tree::{Axis, Pane, PaneId, PaneItem, PaneLookup, PaneNode, PaneTree, SplitId};
...
impl Workspace {
    ...
    /// store.
    fn move_split(&mut self, split: SplitId, ratio: f32, cx: &mut Context<Self>) {
        if self
            .work_area_mut()
...
fn gpui_axis(axis: Axis) -> gpui::Axis {
    match axis {
...
fn opening_bounds(state: &WindowState, cx: &mut App) -> WindowBounds {
    window_bounds(
