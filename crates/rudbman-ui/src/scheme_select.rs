...
//!
//! The same control as [`Select`](crate::Select) — trigger, deferred list,
...
//!
//! The widget knows nothing about what it is previewing: callers hand it plain
//! colors, so the same dropdown fits anything that has a background, a
//! foreground and a handful of accents — the chrome palette of [`theme`] and the
//! syntax palette of [`editor_theme`](crate::editor_theme) alike.
//!
...
/// Height of the trigger, matching [`Select`](crate::Select) so a form that
...
pub struct SchemePreview {
    ...
    ///
    /// Whatever handful of hues tells one scheme from another at a glance: the
    /// status colors of a chrome palette, the keyword and literal colors of a
    /// syntax one. The widget only draws them in the order it is given.
...
pub struct SchemeSwatch {
    ...
    /// which is how a picker offers "use the other catalogue's answer".
...
/// carrying its placeholder label, so that "inherit the other choice" reads as
...
fn preview_pill(preview: Option<&SchemePreview>, placeholder: SharedString, theme: &Theme) -> Div {
    match preview {
        Some(preview) => div()
            ...
            .children(preview.accents.iter().map(|color| {
                div()
                    .flex_none()
                    .size(px(CHIP_SIZE))
                    .rounded_full()
...
impl SchemeSelect {
    ...
    /// Draws the control as a read-only line: muted, unfocusable, and unable to
    /// open however it is told.
    ///
    /// For the case where something else has already made the choice — the
    /// editor theme while it follows the chrome theme — and the answer is still
    /// worth showing, because it moves as the other choice does. Offering a
    /// working dropdown whose every click would be silently discarded would be a
    /// lie, and hiding the row entirely would leave the user guessing which
    /// scheme they ended up with; a disabled trigger says both things at once.
    ...
    ///
    /// Ignored while the control is disabled: a tab stop that cannot be acted
    /// on only lengthens the ring.
    ...
    /// that a flag left standing from before the control was disabled cannot
    /// paint a list nothing can dismiss.
...
impl RenderOnce for SchemeSelect {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let shows_list = self.shows_list();
        let open = self.open;
        ...
        let trigger = div()
            ...
            .text_size(px(14.))
            .line_height(px(20.))
            // Everything that answers a pointer or the keyboard hangs off this
            // one branch, so a disabled control is inert rather than merely
            // grey: nothing to hover, nothing to click, nothing to tab to.
            .when(!disabled, |this| {
                this.cursor_pointer()
        ...
        div()
            ...
            .children(shows_list.then_some(overlays))
            .child(trigger)
    }
}
...
mod tests {
    ...
    /// one that has none.
    fn colors() -> SchemePreview {
        SchemePreview {
            background: hsla(0., 0., 0.1, 1.0),
            foreground: hsla(0., 0., 0.9, 1.0),
            accents: vec![hsla(0.5, 0.8, 0.6, 1.0)],
        }
    }

    /// A swatch with no colors is the "somebody else decides" row, and it has a
    /// label of its own even when the caller supplies none.
    #[test]
    fn a_swatch_without_a_preview_is_a_placeholder() {
        let swatch = SchemeSwatch::new("inherit", "Follow");
        ...
        let swatch = SchemeSwatch::new("one-dark", "One Dark").preview(colors());
        assert!(swatch.preview.is_some());
        assert_eq!(swatch.id, "one-dark");
        assert_eq!(swatch.name, "One Dark");
    }

    /// The state a freshly built control is in, since every one of these is
    /// something the parent is expected to hand over on each render.
    #[test]
    fn a_dropdown_starts_closed_empty_and_enabled() {
        let select = SchemeSelect::new("scheme");
        assert!(select.options.is_empty());
        assert!(select.selected.is_none());
        assert!(!select.open);
        assert!(!select.disabled);
        assert!(!select.shows_list());
    }

    /// The point of [`SchemeSelect::disabled`]: the trigger still reports what
    /// was chosen, but a stale open flag cannot drop a list over the form that
    /// nothing on screen could then dismiss.
    #[test]
    fn a_disabled_dropdown_never_shows_its_list() {
        let select = SchemeSelect::new("scheme")
        ...
        let select = select.disabled(true);
        assert!(select.open, "the parent's flag is left as it was");
        assert!(!select.shows_list());
