...
//! SQL buffer is a table name or a column name, and a regex engine here would
//! be 1.5MB of DFA machinery for a feature the crate below this one
//! (`rudbman-sql`) deliberately went without. If a pattern language is ever
//! wanted it goes in as an option beside this, not instead of it.
//!
...
impl FindState {
    ...
    /// has, and the alternative — rescanning a 100MB buffer per replacement —
    /// is not one.
...
mod tests {
    ...
    fn matching_is_case_sensitive_when_asked() {
        ...
        assert_eq!(find_all(text, "select", true), vec![7..13]);
        assert_eq!(find_all(text, "select", false), vec![0..6, 7..13, 14..20]);
    }
    ...
    fn an_empty_query_matches_nothing() {
        assert!(find_all("select", "", false).is_empty());
        assert!(find_all("select", "", true).is_empty());
    }

    #[test]
    fn matching_lands_on_character_boundaries() {
        let text = "선택 SELECT 선택";
        let found = find_all(text, "선택", false);
        assert_eq!(found.len(), 2);
        for range in found {
            assert_eq!(&text[range], "선택");
        }
    }
