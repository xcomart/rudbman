...
//! wrong text. The single-line field in `rudbman-ui` converts by walking the
//! string; this one cannot, and [`crate::buffer`] explains what it does
//! instead.
//!
//! # No `DisplayMap`
//!
//! `rudbman-ui`'s [`TextInput`] keeps a `DisplayMap` between "the bytes stored"
...
pub enum EditorEvent {
    ...
    /// Execute the statement the caret is in.
    RunStatement {
        /// Where that statement is, exactly as `rudbman_sql::statement_at`
        /// cuts it: `sql_range` is what to send, `range` is what to highlight.
        span: StatementSpan,
    },
...
/// A multi-line SQL editor, as a gpui entity.
///
/// ```ignore
...
///
/// Call once during application start-up, after `rudbman_ui::init`. Everything
...
impl EditorView {
    ...
    /// Sets the SQL dialect, which decides what a keyword and a comment are.
    pub fn dialect(mut self, dialect: Dialect) -> Self {
        self.highlighter.set_dialect(dialect, &self.buffer);
        self
    }
    ...
    /// Changes the SQL dialect and re-lexes the buffer.
    pub fn set_dialect(&mut self, dialect: Dialect, cx: &mut Context<Self>) {
        self.highlighter.set_dialect(dialect, &self.buffer);
        cx.notify();
    }
    ...
    fn toggle_comment(&mut self, _: &ToggleComment, _: &mut Window, cx: &mut Context<Self>) {
        let prefix = syntax::line_comment(&self.highlighter.dialect());
        let (first, last) = syntax::line_span(&self.buffer, &self.selected_range);
    ...
    ///
    /// The same answer `rudbman_sql::statement_at` gives for the whole buffer;
    /// [`crate::syntax`] is where the windowing that makes it affordable lives.
    pub fn statement_at_caret(&self) -> Option<StatementSpan> {
        syntax::statement_at(&self.buffer, &self.highlighter, self.caret())
    }
    ...
    pub(crate) fn brackets(&self) -> Option<(usize, usize)> {
        syntax::bracket_pair(&self.buffer, &self.highlighter, self.caret())
    }
