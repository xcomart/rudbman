...
- 테스트 규모(2026-08-06): Rust 약 710 + Java 115. 전부 실제 JVM/H2를 부팅하는
  통합 테스트를 포함한다.

## 다음 작업

1. **M6 — 백업·DB→DB 전송**: M4가 만든 job 프레임(§6)을 그대로 재사용한다.
   브리지에 `TransferJob`/`BackupJob`을 더하고 진행률 UI는 추출 다이얼로그의
   폴링 패턴을 따른다.
2. **M7 — 쿼리 빌더, 마무리**: ERD 캔버스(팬·줌·드래그·히트 판정)를 공유한다
   (§7.7). PNG 내보내기는 여전히 SVG만(§12.5).
