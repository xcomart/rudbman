...
- 테스트 규모(2026-08-05): Rust 약 660 + Java 115. 전부 실제 JVM/H2를 부팅하는
  통합 테스트를 포함한다.

## 다음 작업

1. **M5 — ERD**: 모델·레이아웃·캔버스·SVG 내보내기. 열린 결정: 자동 배치
   (Sugiyama 자체 구현 vs 크레이트, §12.4), PNG 내보내기는 SVG만으로 시작
   (§12.5). `PaneItem::Erd` 변형 추가가 UI 쪽 진입점.
2. **M6 — 백업·DB→DB 전송**: M4가 만든 job 프레임(§6)을 그대로 재사용한다.
   브리지에 `TransferJob`/`BackupJob`을 더하고 진행률 UI는 추출 다이얼로그의
   폴링 패턴을 따른다. M5와 선후는 자유.
3. **M7 — 쿼리 빌더, 마무리**.
...
- **이 X 디스플레이에서 gpui 앱은 키보드·마우스 입력을 전혀 받지 못한다**
  (포인터 장치가 없어 XInput2 초기화 실패). GUI 확인은 ① 환경변수로 게이트한
