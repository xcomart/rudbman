...
the application shell moved to rugpui).
...
- Test count (2026-08-25): roughly 800 Rust plus 141 Java. It fell by 256 and
  ...
  82 and `rudbman-grid`'s 53 are rugpui's suite now, against the dialect
...
- **Merge with a merge commit once CI is green on all three platforms**
  (`gh pr merge --merge`).
- The four patched gpui crates are rugpui's now, not this repository's, and they
