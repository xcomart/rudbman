...
These are the manifests that put rudbman in the Windows Package Manager, so that
`winget install Xcomart.Rudbman` works. They live here rather than only in
...
package it can find, upgrade or uninstall. `packaging/windows/rudbman.iss`
...
|---|---|
| `Xcomart.Rudbman.yaml` | The version manifest — the identifier, the version, and which locale is the default |
| `Xcomart.Rudbman.installer.yaml` | The download URL, its SHA256, the installer type and scope, and the `ProductCode` |
| `Xcomart.Rudbman.locale.en-US.yaml` | Everything a human reads: publisher, license, description, tags |
...
```powershell
$ver = "0.2.1"
$url = "https://github.com/xcomart/rudbman/releases/download/v$ver/rudbman-v$ver-x86_64-pc-windows-msvc-setup.exe"
Invoke-WebRequest -Uri $url -OutFile "$env:TEMP\rudbman-setup.exe"
(Get-FileHash -Algorithm SHA256 "$env:TEMP\rudbman-setup.exe").Hash
```
...
```powershell
winget validate --manifest packaging\winget\0.2.1
```
...
```powershell
winget install --manifest packaging\winget\0.2.1
```
...
```powershell
winget list Xcomart.Rudbman
winget uninstall Xcomart.Rudbman
```
...
```powershell
$dst = "<fork>\manifests\x\Xcomart\Rudbman\0.2.1"
New-Item -ItemType Directory -Force $dst
Copy-Item packaging\winget\0.2.1\*.yaml $dst
```
...
```
wingetcreate update Xcomart.Rudbman --version <version> --urls <setup.exe url> --submit
```
...
`packaging/windows/rudbman.iss`'s `AppId` with `_is1` appended, the suffix Inno
Setup adds when it writes its uninstall registry key. It is the only thing that
lets winget match an installed copy of rudbman to this package — which is why
`AppId` in `rudbman.iss` is frozen. Change it and every existing installation
...
updater of its own (`rugpui-shell`, given rudbman's endpoints and this
...
release ahead for as long as rudbman stays open. In the rarer case where the
